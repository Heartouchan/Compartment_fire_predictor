...
def compute_hrr_curve(A, t_alpha, Y, dt=1):
    ...
    # decay (quadratic decay: 1 / (x * (t - y)^2))
    area_decay = A - (area_grow + area_steady)
    t_dec = t_now

    if area_decay > 0 and Y > 0:
        y_param = t_dec - (area_decay / Y)
        x_param = Y / (area_decay ** 2)

        while t_now <= 18000:
            denom = x_param * (t_now - y_param) ** 2
            hrr_now = 1.0 / denom if denom != 0 else Y
...
def main():
    ...
    for _, row in data.iterrows():
        ...
        # t_alpha = C2
        Y = C3 * 1.4 * R * total_enclosure_area
        # print(C1)
